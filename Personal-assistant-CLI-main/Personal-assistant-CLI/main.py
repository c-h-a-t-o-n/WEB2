...
class WebUI(UI):
    def input(self):
        # Реалізація логіки веб-введення
        pass

    def output(self, message):
        # Реалізація логіки веб-виводу
        pass

class GUI(UI):
    def input(self):
        # Реалізація логіки введення GUI
        pass

    def output(self, message):
        # Реалізація логіки виводу GUI
        pass
...
class Application:
    ...
    def run(self):
        while True:
            user_input = self.ui.input().lower()

            if user_input in ["good bye", "close", "exit"]:
                self.ui.output("Good bye!")
                break
            elif user_input == "show all":
                self.ui.output(str(address_book))
                continue
            elif user_input == "hello":
                self.ui.output("How can I help you?")
                continue

            items = user_input.split(" ")
            handler_name, *args = items

            handler = Commands.get(handler_name)
            if handler:
                self.ui.output(handler(*args))
            else:
                self.ui.output("No such command")
...
Commands = {
    ...
    "info": print_contact_info

}


if __name__ == "__main__":
    ui = ConsoleUI()  # Можливо замінити ConsoleUI на WebUI або GUI
    app = Application(ui)
    app.run()